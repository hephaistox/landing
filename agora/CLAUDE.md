...
3. **Close = publish** — every draft in the publication is published **at once, in dependency order** (folding in the publish invariant), `publication_id` clears, the publication goes `closed`, and those lineages revert to **classical latest-published TNLR** resolution. Replaces today's per-KI publish button.

So today's per-document publish becomes a **per-publication** publish, and the shipped draft + publish-invariant work folds in: a publication *is* the draft cluster, closed/published in dependency order.
...
- Publication document type (`object_type = "publication"`, not a new table) — gathers a user's pending drafts; exact-id pins while open, a left panel showing its modified objects in context, and **close = publish** (replaces the per-KI publish button; publishes every draft in the publication at once, in dependency order, then pins revert to classical latest-minor TNLR). No epistemic kind; the cross-user objection variant is deferred with the social layer. See §5 "Publication — the document type that gathers drafts"
