...
The `T` in the identity tuple is the **object type**, not the epistemic kind above. The store is single-table and polymorphic (PLM-style): **AGORA_DOCUMENT** holds **KIs**, **Article** and **Publication** — different object types with their own lineages, standing in the same table. Identity is **ObjectType + Name + Lang + Major + Minor**, with `object_type = "ki"` / `"article"` / `"publication"`. A **Publication** is the PLM "change" analog (see §5 "Publication — the document type that gathers drafts"): it gathers a user's pending **drafts** and publishes them together. It has **no epistemic kind**; the cross-user *objection* variant is deferred with the social layer. `Lang` (the content language) is part of identity: each language is its own independent lineage, and the language versions of one concept are tied together simply by sharing a `Name` (see "Language & Translation"). The epistemic `kind` is deliberately *not* in identity.
...
Bibliographic provenance is **two KIs**, not a side table — both `:bibliographic?`, both `object-type :ki`:

- A **work** (`kind=work`, `:inputs? false`) is the reusable source — a book, magazine, website, whatever. Its `content` holds the `:title` plus the cited author `:author-id` (a person → `AGORA_USER`, possibly a login-less `external` one), `:year`, `:editor`, `:url`. It is a **leaf** (no inputs), and its byline is the **cited author**, not its creator (see attribution below).
- An **extract** (`kind=extract`, `:inputs? true`) is one cited passage from a work. Its **single structural input is the work** — pinned like any edge but authored through a picker and shown in the opening prefix, **not written into the prose** — plus a `:locator` (page / verse / entry) in `content`; its `:text` is the quoted passage. On read `engine/resolve-work` turns the extract's first pin into the work's display fields + the extract's own locator, exposed as **`:work`**.

One work → **many** extracts (a magazine → one extract per cited entry), so a work lists all its citations. The kinds chain by `:consequence`: a **work**'s consequence-button spawns an **extract**, an **extract**'s spawns a **definition** — provenance flowing into appropriation.

**The single structural input** (an extract's work; likewise an illustration/counter-example's claim) lives **out of the prose**: it is the kind's one input, pinned and successor-indexed, authored via a picker and rendered in the statement prefix. Editing the body never adds or drops it — `engine/resolve-work`/`resolve-target` recover it from the first pin, and the ✕ on the input card removes it by stripping its `[[…]]` from the text.

**Attribution belongs to whoever the document is *about*, not always its owner.** `document.kind/attributed-author-id` (the linkable id) resolves in two levels: for a **work** itself, its cited author (`:author-id`) → else the document's own owner; `attributed-author` (the name) reads the derived `computed.:author`, cached from that person's account. The read view exposes them as **`:attributed-author` / `:attributed-author-id`**, so a byline's name and its profile link always point at the same person: the cited author for a work, the owner otherwise. A document that merely **cites** an extract is **not** attributed to the work's author; the underlying work is shown **on that input** (`engine/resolve-input` attaches `:work`) where the reader sees it. Works and extracts render on the page and are emitted as schema.org `citation`.

### Definitions vs extracts — fidelity vs appropriation

`work`/`extract` and `definition` are **distinct kinds related by a citation edge, never merged** — they answer different questions, so the "unify vs special-case" call lands on *two concepts, one edge between them*.

- An **extract** is *faithful provenance*: what a work says at a given locator, in the book's **own** context and vocabulary. Its job is fidelity — quote the passage accurately, pinned to page/verse. It is challenged on accuracy, edition, attribution.
- A **definition** is the writer's *appropriation*: the concept lifted out of that book's vocabulary and re-expressed as a semantic contract that holds **inside Agora's** vocabulary. The "fine-tuning" is porting a term across contexts — narrowing it, disambiguating it against the graph's other uses, reconciling it. The definition is where the writer's judgment lives; the extract is where the book's authority lives. It is challenged on definitional grounds (an incompatible definition of the same term).

The `:consequence` chain expresses this directly: **an extract's consequence-button creates a `definition` that cites it**. Two refinements settled in design:

1. **Work-grounding is optional, not constitutive.** Many definitions are pure *stipulation* — the writer fixes a term for the graph with no book behind it. So "a definition is what you keep from a work" is the *grounded* case (the strongest one), not the only one; the `definition` kind stays general. The richest case is a definition reconciling **several** extracts into one contract — the fine-tuning *is* that reconciliation.
2. **The extract→definition edge is provenance, not a logical premise.** A definition isn't *derived* from its extract the way an inference is derived from its inputs — you can't refute a definition by finding a counterexample in the book; the extract is *authority / where-it-came-from*, not a reasoning step. So although a cited extract rides in `:inputs` today, it semantically belongs to the **annotation/reference** family, not the reasoning family — more evidence that the reasoning-vs-annotation edge split (Layer 3, #79) is real: "what grounds this?" and "what does this logically follow from?" are different edges.
...
### Publication — the document type that gathers drafts (Layer 2)

A **Publication** is the PLM change-analog, named to reuse the existing *publish/published* vocabulary rather than invent "change". It is **not a separate table** — it is another **object type in `AGORA_DOCUMENT`** (`object_type = "publication"`), exactly as *Article* differs from *KI*, and **no epistemic kind**. It is **versioned like any document**: its stable `name` (cid) is the identity, and each change to it is an immutable **new minor** — a **rename** and the **close-on-publish** both bump the minor. Its `content` is `{:title :status :owner-id :published-at}` (its byline name, like any document's, is derived into `computed.:author`); `:status` (`:open` / `:closed`) rides in content, and the mutable `draft` column mirrors it (open ⇒ draft). A publication has **no content language** — it is a container resolved by cid alone, so its NOT-NULL `lang` column holds a neutral placeholder (`:zz`), never a real code (`landing.agora.publication`).

A Publication **gathers pending drafts** — the Layer 1 **draft** is the atom, the publication is the aggregate (e.g. a new KI plus the inline predecessors/successors created alongside it — a whole draft cluster). Every KI/article create or edit happens *inside* an open publication and tags its draft (`publication_id`); the publication is the one thing created *outside* a publication. A publication is provisioned with **no user interaction** — authoring with none active auto-creates one, Word-style (a blank title is auto-named `publication<N>`, editable via rename). The **active publication** (the one create/edit attach to) is a client-side choice persisted in localStorage, shown as a header chip and set from a publication's page ("Work in this publication"). The publication's **page** lists the documents it gathers (its drafts) as a card grid — the staging view — reached from the header **Publications** entry (an index of your open publications).

**Lifecycle (edition mechanism — single-user):**

1. **Created & owned** by the user who opens it (author + sole owner); status `:open`.
2. **Pins are exact while the publication is open** — its drafts and their inputs resolve to **specific version ids** (exact UUID), not the floating latest-minor (publication-scoped resolution, #83).
3. **Close = publish** — every draft the publication gathers is published **at once** (`draft` flipped to published in one transaction), the publication closes (a new minor, status `:closed`), and those lineages resolve **classically** (latest published minor). `publication_id` is **kept** on the published versions as permanent provenance (so a closed publication's page still lists what it published). Owner-only, from a top-level **Publish** button on the publication page. Replaces the per-KI publish button.

So today's per-document publish becomes a **per-publication** publish — **atomic**: all its drafts flip to published in one transaction. There is **no dependency ordering to enforce**: a draft can only cite already-published documents or **other drafts in the same publication** (publication-scoped resolution never surfaces another open publication's drafts), so publishing the whole cluster at once always yields a consistent state. The publish invariant (a published document's inputs are all published) therefore holds **by construction**, not by sequencing — the guarantee that keeps it true is the citation rule, enforced at authoring time (#105), that you can only cite the published corpus or the current publication's own drafts.

**Deferred (social layer):** the cross-user **objection** variant — someone raising a challenge/proposal against another owner's KI, the handoff where authoring passes to the target owner, and reject/accept — plus fork/merge and notifications. Whether an objection is a *kind* of publication or a separate mechanism is revisited then.
...
- **At authoring time (UI):** the citation editor's search **removes the current document from its results**, so a KI can never be made to cite itself (`cite/citation-editor`'s `self-name` argument). Dangling references can't normally be authored because you cite by picking an existing KI (or creating one inline).
...
> - **Sources** — bibliographic provenance is two KI kinds, not a side table: a **work** (`kind=work`, the shared source — cited author + title/year/editor/url in content, a leaf) and an **extract** (`kind=extract`, one cited passage — the work is its single structural input, locator in content). Resolved on read (`engine/resolve-work`), attributed to the cited author, emitted as schema.org `citation`. See "Sources & References".
> - **Epistemic types are self-hosted** — each `kind` is described by a `definition`-kind KI (slug `type-<kind>`); the kind→definition link is domain data (`document.kind/kind-def`).
...
- Publication document type (`object_type = "publication"`, not a new table) — gathers a user's pending drafts; exact-id pins while open, a publication page listing its modified objects, and **close = publish** (replaces the per-KI publish button; publishes every draft at once, then those lineages resolve classically). No epistemic kind; the cross-user objection variant is deferred with the social layer. See §5 "Publication — the document type that gathers drafts"
...
- **Tables** (`resources/agora/schema.sql`, applied manually via `scripts/agora_db.clj`): `AGORA_DOCUMENT` (every document — one polymorphic table; `type` ∈ {ki, article, publication}, where a bibliographic `kind=work`/`kind=extract` KI is a source/citation), `AGORA_SUCCESSOR` (derived reverse-edge cache), `AGORA_USER` (accounts **and** login-less `provider='external'` cited people). All text stored inline; no blob store.

### Read model — document + caches (reads ≫ writes)

Graph traversal and version resolution are **precomputed on write and cached**, never done in SQL per read. Engine: `landing.agora.document` (+ `landing.agora.node`, the SQL/cache adapter; `landing.agora.cache`, Caffeine). Pure identity/edge rules (TNLR, cid/slug, citations, pins, successors) live in `landing.agora.document.identity`; kinds and the statement/prose grammar in `landing.agora.document.kind` (both cljc).

- **TNLR** = (Type, Name, Lang, major Release) — identity minus `minor`; names a lineage whose latest minor is current.
- **Two resolution modes for a TNLR.** (1) *Public / permalink* — **latest *published* minor** (`draft = 0`); a **draft-only** lineage does **not** resolve (its permalink 404s), keeping drafts out of public views. (2) *Publication-scoped* — while authoring or reading **in a publication**, a lineage resolves to **the publication's own draft** of it if it has one, else the latest published minor. This is `db-doc/latest-of` = `(or (draft-of … pub-cid) (latest-published-id …))`, used by `pin-all` on **write** (given the `publication_id`) and by `resolve-in-publication` on **read** (reached via `?publication=<id>`). So a version authored in a publication pins to that publication's in-progress inputs and reading in it shows the draft graph, while public reads (no publication context) stay on mode 1. There is **no global draft fallback**: because every create/edit happens inside a publication (enforced, §12 slice 6) and the publish invariant holds, a draft is only ever reachable inside its own publication. On publish, `publication_id` is kept as provenance but the versions become published, so resolution reverts to classical. See §5 "Publication — the document type that gathers drafts".
- **A document is a row keyed by `id`.** Columns are only the identity keys (`id`, `type`, `name`, `lang`, `major`, `minor`) so indexes never parse EDN. Everything else is two EDN blobs: **immutable `content`** (`{:kind :title :text :owner-id :published-at :inputs [TNLR…]}`, never updated after insert) and **mutable `computed`** (`{:pins [<input TNLR + resolved :id>…] :author "<byline name>"}`, built by `db-doc/computed`). `:pins` is a **vector** produced by `pin-all` — each entry is one input TNLR plus its concrete predecessor `:id`, so it doubles as the API-facing input refs on read (no separate zip step). A KI's "statement" and an article's "body" are the same `:text` slot.
- **The byline name is derived, the person is stored.** `content` holds only *who* — `:owner-id`, plus a work's cited `:author-id` — and `computed.:author` caches the `AGORA_USER.display_name` of `(or :author-id :owner-id)`, re-derived at every write and repaired by the reconcile. A name in the immutable blob could never be rectified or erased (RGPD art. 16/17); in the derived blob, renaming or anonymizing a person propagates to every document they signed.
...
- **References** are resolved on read by `engine/resolve-work`: an extract's first pin → the work's display fields + its own locator, exposed as `:work` (fetched through the cached storage, like any input). Their authors are `AGORA_USER` people. Confidence, once computed, also lives in mutable `computed` — never in the immutable content.
...
**Design is dialogic; a real objection beats fast agreement.** The owner reasons out loud, proposes, revises, and converges *through* the exchange ("the more you speak about it, the clearer it is to me"). Propose with a recommendation and its trade-offs rather than executing the literal ask; when asked "do you have any objections?", give a genuine critique. Sycophancy actively wastes their time. **The owner has said this explicitly: they *prefer* being argued with over easy agreement.** So when you disagree, say so up front with reasons — don't nod along and build it. Anchor the objection in the owner's own principles when you can (ubiquity, derive-over-store, domain-first); then still offer to do it their way if they hold the view after hearing you. (Example: they asked to clone the cljc domain into `-old` copies; the objection — two sources of truth, a forked frontend↔backend contract — was welcomed and changed the plan.)
