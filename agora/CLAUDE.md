...
Beyond `[[ki:…]]` **inputs** (which cite other KIs), a document may cite **one external source** — bibliographic provenance. As of Layer 1A a **source is a first-class document** (`type="source"`, not a separate table): the shared *work* (a book/article), `content = {:kind "source" :title :author :owner-id :year :editor}`, whose **owner is the cited author** (a person in `AGORA_USER`, usually a login-less `external` like "Sun Tzŭ"). So a source is versioned/translatable like any document, and — later — objectable. Two sources are two pieces of knowledge → two documents; combining them is an inference that stands apart (hence **0 or 1** source per document, never a list).

The **citing** document holds a single reference on *its own* side: `content.:source = {:name <source-cid> :major :locator}` — the shared work (by cid) plus a **per-citation locator** (page / chapter / verse). The book is the shared entity; the locator belongs to the KI. It is **resolved on read** (`source/resolve-ref`) to the display fields `{:id :title :year :editor :author-name :author-id :locator}` — the same shape the page/SEO already consume — so the reference is version-current (the book is shared; editing which source or which locator is a new minor of the KI, editing the book is a new minor of the *book*). This replaces the earlier frozen-snapshot model (`content.:source` was a copy of the source's fields) — version-currency now comes from resolving the shared document, not from freezing.

Attribution flows from the source: a KI's kind-guided opening ("Sun Tzŭ believes that …") takes its subject from the cited source's author (`document-domain/attributed-author` → `:author-name` of the resolved source, else the KI's own author). Authoring: search existing sources (author/title/year), reuse recent ones in one click, or create a new source + author-person inline (create = `document/create "source"`; the client contract `{:source-id :locator}` is unchanged). The source renders on the page (author → profile), attributes the cited author on discover cards, and is emitted as schema.org `citation`. Sources are **excluded from the sitemap** (no public shell of their own yet).
...
- **Tables** (`resources/agora/migrations/`, applied manually via `scripts/agora_db.clj`): `AGORA_DOCUMENT` (KIs, articles **and** sources — one polymorphic table; `type` ∈ {ki, article, source}), `AGORA_SUCCESSOR` (derived reverse-edge cache), `AGORA_USER` (accounts **and** login-less `provider='external'` cited people). `AGORA_SOURCE` was **dropped in Layer 1A** (migration `005`) — sources are now `type="source"` documents. All text stored inline; no blob store.
...
## 16. How this collaboration works (from Claude's point of view)

Written by the implementer for the next implementer. This is not process ceremony — it is the shortest path to being useful here.

**Design is dialogic, not dictated.** The owner reasons out loud, proposes, revises, and reaches clarity *through* the exchange ("the more you speak about it, the clearer it is to me"). Decisions are converged on, rarely handed down. So: propose with a recommendation and its trade-offs, don't just execute the literal ask; a good objection is worth more than fast agreement. When asked "do you have any objections?", give a *real* critique — the source-as-document design was sharpened by pushing back on the locator problem and on "persons don't go away," not by nodding. Sycophancy actively wastes the owner's time.

**The house design instinct: unify and derive; resist special cases and premature structure.** Every hard question here has been answered the same two ways — *collapse a special case into the general model*, or *derive a value instead of storing it*. KIs and articles became one polymorphic table; sources became documents; the statement prefix is derived not stored; the URL slug is derived from the title; the cited source is resolved on read, not snapshotted. Corollaries the owner has affirmed by rejecting the alternative: **facades over a central config** (a central `config.cljs` was rejected), **flat over nested** (disjunctive antecedents were rejected), **immutable + versioned everywhere**, **one mechanism with parameters over N branches**. When scoping anything new, ask first: *can this be a document? can this be derived?* That is usually the intended answer.

**Execution latitude scales with the signal — read it.** The default cadence is one vertical slice at a time, stop for review; the owner commits and closes issues themselves (`Closes #NN`). But they escalate explicitly ("go really ahead, not one step") and then tolerate breaking changes, because they run their own **code-review + testing sessions**. Match that: don't over-checkpoint once they've said go; don't run ahead of an un-escalated task. Corrections are precise and craft-level (a prefix on its own line; a card showing a cid instead of a title) — fix the *root cause*, not the symptom, and expect them to supply concrete design when they hold a view (they proposed the inline-boxed prefix themselves).

**Operational discipline is theirs, and the environment is constrained.** Compile through the **running nREPL (port 7051)** — never spawn app JVMs; the DB is a shared 5-connection Clever addon and a second pool starves it. Migrations and seeds are raw-SQL / REPL, applied manually, and live in `env/seed` (`:env-seed`); dev points at the shared prod DB. In dev, **data is disposable** ("no data is important") — prefer reseed over a data migration when it is cheaper. Applying migrations/seeds is normally the owner's move; they delegate it explicitly when they want you to. The issue tracker is the spine of the work — file issues freely; they curate and close them.

## 17. The architecture, in one mental model

If you internalize one thing, internalize this: **everything is a versioned document, and the engine never knows the type.**

- **One spine.** `AGORA_DOCUMENT` is a single polymorphic table (`type` ∈ `ki | article | source`, `objection` later); `landing.agora.document` is one engine over it that *never branches on a specific type*; thin per-type **facades** supply the only type-specific data (the strings `"ki"`/`"article"`/`"source"` live only there). Read that sentence back before adding a feature — the answer is usually "make it the engine's problem, parameterised," not "add a case."
- **The recurring move is unify + derive** (see §16). New capability → first ask whether it is a document, and whether the value can be derived rather than stored. Confidence is *computed*; the successor index is a *derived cache*; the statement prefix and URL slug are *derived*; the source display is *resolved*. Storing a denormalized copy is the exception that must justify itself.
- **Identity is layered so the stable parts never move and the human parts move freely.** Identity = TNLR (Type, Name, Lang, Release) + minor. `Name` is an **opaque immutable cid** — it is the reference key for every citation/pin/edge and never changes; the **title** is editable; the **URL** is a decorative `<cid>~<title-slug>` resolved by cid alone. Language is *part of* identity (siblings share a cid = "translation-by-name"); the epistemic **kind is a mutable attribute, deliberately not identity** (reclassifying is an edit, not a fork).
- **The read model is immutable `content` + mutable `computed`, cached.** Columns are only the identity keys; everything else is two EDN blobs — authored `content` (never rewritten) and `computed` (`:pins`). Graph traversal and version resolution are **precomputed on write and served from Caffeine caches**; a read never walks the graph in SQL. `AGORA_SUCCESSOR` is a rebuildable derived cache, not a source of truth.
- **`document-domain.cljc` is the anti-drift device.** All pure graph/identity/statement logic — what a citation, a pin, a cid, a slug, a statement prefix *is* — lives once, shared clj + cljs, so the server and the SPA cannot disagree. If a rule must hold on both sides, it belongs there and nowhere else.
- **Edges are citations are inputs.** An in-text `[[ki:cid@major]]` token *is* an input edge *is* a graph edge — one grammar (`domain/cite-pattern`), parsed on write. The single **source** reference (typed, 0-or-1, carrying a locator, resolved on read) is the one deliberate exception to "edges live in the prose."
- **Both stacks are layered the same way:** backend `document-store` (SQL + cache adapter) → `document` (engine) → `endpoints`; frontend `document-page` (type-agnostic engine) → `view`/`edit` → `ki-page`/`article-page`/`author-page` facades. Cross-cutting concerns — **SEO** (server-rendered head + body on public permalinks, replaced by the SPA on mount), **i18n** (content-language in identity + a separate interface-language preference), **caching** — sit orthogonally across those layers.
- **The telos is the constraint that explains the shape:** *store the reasoning, and make every part of it challengeable.* That is why sources became documents (so they can be objected to), why immutability + versioning is pervasive, and why nothing is a dead-end leaf. The unbuilt Layer 2 (objections, fork/merge, confidence) is already **anticipated in the data model** — respect that when extending it.
