...
**Attribution belongs to whoever the document is *about*, not always its owner.** `document.kind/attributed-author` (name) + `attributed-author-id` (linkable id) resolve in two levels: for a **work** itself, its cited author (`:author-id`) → else the document's own owner. The read view exposes them as **`:attributed-author` / `:attributed-author-id`**, so a byline's name and its profile link always point at the same person: the cited author for a work, the owner otherwise. A document that merely **cites** an extract is **not** attributed to the work's author; the underlying work is shown **on that input** (`engine/resolve-input` attaches `:work`) where the reader sees it. Works and extracts render on the page and are emitted as schema.org `citation`.
...
- **A document is a row keyed by `id`.** Columns are only the identity keys (`id`, `type`, `name`, `lang`, `major`, `minor`) so indexes never parse EDN. Everything else is two EDN blobs: **immutable `content`** (`{:kind :title :text :author :owner-id :published-at :inputs [TNLR…]}`, never updated after insert) and **mutable `computed`** (`{:pins [<input TNLR + resolved :id>…]}`). `:pins` is a **vector** produced by `pin-all` — each entry is one input TNLR plus its concrete predecessor `:id`, so it doubles as the API-facing input refs on read (no separate zip step). A KI's "statement" and an article's "body" are the same `:text` slot.
