...
Two layers: a **source** (the reusable *work* — a book, magazine, website, whatever) and a **citation** (one idea from it). A **source** is an `AGORA_SOURCE` row `{id, person_id (author→AGORA_USER, may be login-less external), title, year, editor, url}` — shared, edited in place. A **citation is a `kind=source` KI**: its `:text` is the cited idea, its `content.:source = {:source-id :locator}` references the work + this citation's own locator (page/verse/entry), and — because `kind=source` declares `:inputs? false` — it takes no inputs (a leaf). One source → **many** `kind=source` KIs (e.g. a magazine → one KI per cited ranking/entry), so you can list all citations of a work. On read, `source/resolve-ref` turns `content.:source` into the work's display fields + locator.

**A normal KI relates to a source only by *citing* it** — never by "attaching" one. Citing uses the **classical citation mechanism** (the cite-search box), with one difference declared in the domain: `kind=source` sets **`:in-text? false`** (`document.kind/kind-cites-in-text?`), so a source citation is an **input edge only** — it is *not* written into the prose. The read layer splits these `kind=source` inputs out of `:inputs` into a resolved **`:cites`** list (title + work-author + locator); the client sends them as `:cites [{:name :major}…]` (transient — `document/inputs-of` folds them into `:inputs`, and the editor resubmits the full list each edit since they aren't in the text to re-derive).

**Attribution belongs to whoever authored the document.** The byline person is derived by a matched pair, `document.kind/attributed-author` (name) and `attributed-author-id` (linkable id), each two levels: the `:source`'s author/author-id (for a `kind=source` KI *itself* — its `:source` resolves to the cited work) → else the document's own `:author`/`:owner-id`. The read view exposes them ubiquitously as **`:attributed-author` / `:attributed-author-id`** (dropping the raw `:author`/`:owner-id`), so a byline's name and its profile link always point at the same person: the cited author for a source KI, the owner otherwise. A KI that merely **cites** a source is **not** attributed to that source's author; the cited author is shown **on the source input** (`:cites`, via `source/cites-view` — "title · cite author · locator") where the reader sees it. Authoring: the source-editor (pick/create a work + locator) shows **only when kind=source**; other KIs get the cite-search box instead. Sources render on the page (author shown on the cite), and are emitted as schema.org `citation`. 
...
- A **source** is *faithful provenance*: what a work says at a given locator, in the book's **own** context and vocabulary. Its job is fidelity — cite/paraphrase the passage accurately, pinned to page/verse. It is challenged on accuracy, edition, attribution.
...
The model already expresses this with no new plumbing: **a definition that grounds itself in a work is simply a `definition` KI that cites a `source` KI** (via the edge-only cite mechanism above). Two refinements settled in design:
...
2. **The source→definition edge is provenance, not a logical premise.** A definition isn't *derived* from its source the way an inference is derived from its inputs — you can't refute a definition by finding a counterexample in the book; the source is *authority / where-it-came-from*, not a reasoning step. So although a cited source rides in `:inputs` today, it semantically belongs to the **annotation/reference** family, not the reasoning family — more evidence that the reasoning-vs-annotation edge split (Layer 3, #79) is real: "what grounds this?" and "what does this logically follow from?" are different edges.
...
- **Tables** (`resources/agora/schema.sql`, applied manually via `scripts/agora_db.clj`): `AGORA_DOCUMENT` (KIs **and** articles — one polymorphic table; `type` ∈ {ki, article}, where a `kind=source` KI is a bibliographic citation), `AGORA_SUCCESSOR` (derived reverse-edge cache), `AGORA_USER` (accounts **and** login-less `provider='external'` cited people), `AGORA_SOURCE` (the shared bibliographic *work* a `kind=source` KI references). All text stored inline; no blob store.
