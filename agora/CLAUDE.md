...
The `T` in the identity tuple is the **object type**, not the epistemic kind above. The store is single-table and polymorphic (PLM-style): **AGORA_DOCUMENT** holds **KIs**, **Article** and **Publication** — different object types with their own lineages, standing in the same table. Identity is **ObjectType + Name + Lang + Major + Minor**, with `object_type = "ki"` / `"article"` today and `"publication"` (Layer 2). A **Publication** is the PLM "change" analog (see §5 "Publication — the document type that gathers drafts"): it gathers a user's pending **drafts** and publishes them together. It has **no epistemic kind**; the cross-user *objection* variant is deferred with the social layer. `Lang` (the content language) is part of identity: each language is its own independent lineage, and the language versions of one concept are tied together simply by sharing a `Name` (see "Language & Translation"). The epistemic `kind` is deliberately *not* in identity.
...
**Attribution flows from the cited source:** a position's kind-guided opening ("David Fricke soutient que …") takes its subject from `document.kind/attributed-author`, priority `:source` author (for a `kind=source` KI itself) → **`:cite-author-name`** (the work-author of the KI's first source input, computed by the read layer) → the KI's own author. Authoring: the source-editor (pick/create a work + locator) shows **only when kind=source**; other KIs get the cite-search box instead. Sources render on the page, attribute the cited author on discover cards, and are emitted as schema.org `citation`. `AGORA_SOURCE` was **dropped in Layer 1A then restored** (migration `006`) — the intermediate "source is a `type=source` document" model was reverted; a source is a `kind=source` KI + a shared `AGORA_SOURCE` work.
