...
(def corpus
  "A tiny multi-lineage corpus: a definition (two minors), an inference citing it (a draft in a
  publication), and the publication document itself."
  [{:type "ki"
    :name "def"
    :lang "en"
    :major 1
...
(deftest graph
  (testing "resolve-latest narrows to a lineage and takes its current minor"
    (is (= 1 (:minor (sut/resolve-latest corpus (t "ki" "def")))))
    (is (nil? (sut/resolve-latest corpus (t "ki" "ghost")))))
  (testing "declared-inputs reads a version's :inputs"
    (is (= [{:type "ki"
             :name "def"
             :lang "en"
             :major 1}]
           (sut/declared-inputs (sut/resolve-latest corpus (t "ki" "claim"))))))
  (testing "resolved-inputs resolves each input to its current version (nil if dangling)"
    (let [ins (sut/resolved-inputs corpus (sut/resolve-latest corpus (t "ki" "claim")))]
      (is (= 1 (count ins)))
      (is (= 1 (:minor (:doc (first ins)))) "resolves to def's latest minor")))
  (testing "publication-of resolves a member's link back to its publication"
    (is (= "pub" (:name (sut/publication-of corpus (sut/resolve-latest corpus (t "ki" "claim"))))))
    (is (nil? (sut/publication-of corpus (sut/resolve-latest corpus (t "ki" "def"))))))
  (testing "members finds the publication's documents at their current version"
    (is (= ["claim"]
           (mapv :name (sut/members corpus (sut/resolve-latest corpus (t "publication" "pub")))))))
