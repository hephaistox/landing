(ns cache-bust
  ...
  (:import
   (java.nio.file CopyOption FileSystems FileVisitOption Files LinkOption
...
(defn- write-str [^Path p ^String s] (Files/write p (.getBytes s "UTF-8") (make-array OpenOption 0)))
      ...
      (let [public   (.getPath fs "/public" (make-array String 0))
            ...
            assets   (for [p files
                           ...
                           :when (and (re-find asset-ext-re n)
                                      ...
                                      (let [t (tail p)]
                                        (some #(str/includes? % t) (vals contents))))]
                       p)
            renames  (doall
                      (for [p assets
        ...
        (doseq [[^Path h content] contents
                :let [updated (reduce (fn [s [from to]] (str/replace s from to))
                                      content
                                      replacements)]
                :when (not= content updated)]
          (write-str h updated))
        (println "cache-bust:" (count renames) "asset(s) fingerprinted,"
                 (count htmls) "HTML page(s) scanned")))))
...
(defn deploy!
  ...
  (let [exit-code (build-fn printers
    ...
    (if (and (number? exit-code) (zero? exit-code))
      (do (fingerprint-jar! (str target-dir "/landing.jar"))
          (push! target-dir)
          exit-code)
      (do (println "cache-bust: build failed (exit" exit-code
                   "); skipping fingerprint + push")
          exit-code))))
