(ns landing.article.privacy
  (:require
   [landing.routes :refer [mail-txt url-txt]]))
...
(defn privacy-body
  ...
  (case l
    ...
    [[:article.text
      ...
      [:h4 "2.2. Ways we collect data from you"]
      [:p
    ...
    [[:article.text
      ...
      [:h4 "2.2. Comment nous collectons les données"]
      [:p
      ...
      [:h3 "5. Protection de l'information"]
      [:p
      ...
      [:h3 "Autres questions"]
      [:p
       "Si vous avez d'autres questions ou des commentaires sur notre gestion des données personnelles, merci de nous contacter, soit par e-mail à "
       (mail-txt :info-mail l)
       " ou par courrier à Hephaistox (cf. adresse ci-dessus)"]]]))
