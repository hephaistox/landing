(ns landing.agora.frontend.source
  "Bibliographic references — authoring + display, shared by the KI and article forms.

  A document's references are `[{:id :title :year :editor :author-name :author-id
  :locator}…]` — each pins an AGORA_SOURCE *work* (`:id`) plus a per-citation `:locator`.
  Three ways to add one: a **recent-source chip** (one click), the **\"Find a source\"
  search modal** (author/title/year filters), or **creating a new source** (with an author
  person-picker). `strip-display` reduces a ref to the `{:source-id :locator}` the API
  stores. Low-level (no dependency on the page namespaces), like `cite`."
  (:require
...
;; --- persistence shape ------------------------------------------------------
(defn strip-display
  "Reduce editor refs (which carry resolved display fields + `:id`) to what the API
  persists: `[{:source-id :locator}…]`. Send `[]` (not nil) to clear all references."
  [refs]
  (mapv (fn [r]
          {:source-id (:id r)
           :locator (:locator r)})
...
;; --- the per-document references editor -------------------------------------
(defn references-editor
  "Manage a document's references. `value` = current refs vector; `set-refs!` gets the
  new vector on every change."
  [_value _set-refs!]
    ...
    (fn [value set-refs!]
      (let [lang @(rf/subscribe [::i18n/lang])
        ...
        [:div
         ...
         ;; current references
         (into [:div]
               (for [[i r] (map-indexed vector refs)]
                 ^{:key (str (:id r) "-" i)}
                 [:div {:style {:display "flex"
                                :align-items "center"
                                :gap "0.5em"
                  ...
                  [:input {:type "text"
                           ...
                           :on-change #(set-refs!
                                        (assoc-in refs [i :locator] (.. % -target -value)))}]
                  [:button {:title (i18n/t lang :ref/remove)
                            :on-click #(set-refs! (into (subvec refs 0 i) (subvec refs (inc i))))
                            :style {:border "none"
                                    :background "transparent"
                                    :color "#c92a2a"
                                    :cursor "pointer"}}
            ...
            (into [:span]
                  (for [s @recent]
                    ...
                    [:button {:style chip
                              ...
                              :on-click #(set-refs! (conj refs (assoc s :locator "")))}
                     (:title s)
                     (when (:year s) (str " " (:year s)))]))])
         ;; find + modal
         ...
         (when @open?
           [source-search-modal
            (fn [s] (set-refs! (conj refs (assoc s :locator ""))) (reset! open? false))
            #(reset! open? false)])]))))

;; --- read-only display on the page ------------------------------------------
(defn references-view
  "Render a document's resolved references under the card (author → profile link,
  title, year, editor, locator). Nothing when there are none."
  [refs]
  (when (seq refs)
    (let [lang @(rf/subscribe [::i18n/lang])]
      [:div {:style {:margin-top "1.1em"}}
       ...
       (into [:ul {:style {:margin 0
             ...
             (for [[i r] (map-indexed vector refs)]
               ...
               [:li
                (if (:author-id r)
                  [:a {:href (i18n/author lang (:author-id r))
                       :style {:color "#b9770e"
                ...
                " · "
                (:title r)
                (when (:year r) (str " (" (:year r) ")"))
                (when-not (str/blank? (:editor r)) (str " · " (:editor r)))
                (when-not (str/blank? (:locator r))
