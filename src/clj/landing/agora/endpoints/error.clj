(ns landing.agora.endpoints.error
  ...
  A database failure — the `:landing.agora.document.store-old/db-unavailable` ex-info that
  `landing.agora.document.store-old` rethrows after logging, or a raw `java.sql.SQLException` from
...
(def exception-middleware
  ...
  (exception/create-exception-middleware
   (merge exception/default-handlers
          {:landing.agora.document.store-old/db-unavailable (fn [_e _req] unavailable)
