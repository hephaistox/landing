...
(defn sha-256
  ...
  [text]
  (let [bytes (.digest (MessageDigest/getInstance "SHA-256") (.getBytes ^String text "UTF-8"))]
...
(defn write-blob
  ...
  (let [hash (sha-256 text)]
    (jdbc/execute! db/ds ["INSERT IGNORE INTO AGORA_BLOB (hash, content) VALUES (?, ?)" hash text])
    hash))

(defn read-blob
  "Return the text stored under `hash`, or nil if no such blob exists."
  [hash]
  (:AGORA_BLOB/content (jdbc/execute-one! db/ds
